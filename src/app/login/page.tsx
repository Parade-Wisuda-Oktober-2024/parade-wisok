...
export default function Login()  {
  ...
  >
        <div className="w-full h-[100%] sm:max-w-2xl sm:h-[75%] px-20 pb-15 pt-40 sm:pt-[0] relative bg-[#3a0758] bg-opacity-35 rounded-lg shadow-lg text-center z-10">
            <div className="relative flex flex-col items-center justify-center h-[100px] sm:h-[50px] sm: mb-2 sm:mt-3 md:h-[60px] md:mb-3 md:mt-4 lg-[80px] lg:mb-12 lg:mt-14 xl:h-[100px] xl:mb-18 xl:mt-18">
                {/* Burung kiri */}
            ...
            <form id="loginForm" onSubmit={handleSubmit(onSubmit)} className="space-y-1 py-2 relative h-[230px] sm:h-[13rem] md:h-[13rem] lg:h-[15rem] xl:h-[15rem]">
                ...
                <input type={showPassword ? 'text' : 'password'} id="password" placeholder="Password"  {...register('password', { required: 'Password is required' })} className="w-full px-4 py-1 font-paragraph text-xl bg-[#8063bb] bg-opacity-60 text-white rounded-md focus:ring-2 focus:ring-pink-400 focus:outline-none"></input>
                <button type="button" onClick={togglePasswordVisibility} className="absolute right-2 top-1 p-1 bg-transparent opacity-70 text-gray-600 hover:text-gray-900">
                  {showPassword ? (<Image src={'/login/view.png'} alt='view password' width={20} height={20}/>):(<Image src={'/login/hide.png'} alt='hide password' width={20} height={20}/>)}
                  </button>
                </div>
        ...
        <Image src={'/login/VIS 4.png'} alt='swirl' width={1500} height={1500} className='absolute top-0 right-1/4 w-full translate-x-[6rem] translate-y-[-1rem] sm:translate-x-[10rem] sm:translate-y-[-5rem] md:translate-x-[14rem]  md:translate-y-[-6rem]  lg:translate-x-[16rem]  xl:translate-x-[21rem] opacity-50'></Image>
        </div>
