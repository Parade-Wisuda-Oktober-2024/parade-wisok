    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-start pt-8 z-30">
        ...
        <div className="relative h-[150px] w-[150px] lg:h-[190px] lg:w-[195px] mb-2">
          {/* Outer frame image */}
          <img src={outerFrame} alt="Outer frmae"
               className="absolute inset-0 h-full w-full object-cover z-10 rounded-lg"/>

          {/* image panitia */}
          <div className="absolute inset-0 z-40 overflow-hidden rounded-lg">
            <Image src={photo} alt="photo" layout="fill" objectFit="cover" objectPosition={isOpras ? 'top' : 'center'}/>
          </div>
        </div>
        ...
        <div className="text-center pb-2">
          <h1
            className="font-header text-lg sm:text-lg lg:text-2xl bg-gradient-to-r from-[#FFF3D9] to-[#FFD787] bg-clip-text text-transparent [text-shadow:2px_2px_10px_#F4D38E]">
            {title}
          </h1>
          <p
            className="font-bold text-lg sm:text-lg lg:text-2xl bg-gradient-to-b from-[rgba(254,229,154,1)] to-[#f3d29b] bg-clip-text text-transparent [text-shadow:2px_2px_20px_rgba(255,191,81,1)]">
            {name}
          </p>
          <p
            className="font-bold text-lg sm:text-lg lg:text-2xl bg-gradient-to-b from-[rgba(254,229,154,1)] to-[#f8dba8] bg-clip-text text-transparent [text-shadow:2px_2px_20px_rgba(255,191,81,1)]">
            {batch}
          </p>
        </div>
      </div>
